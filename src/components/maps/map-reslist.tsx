...
const MapResList = ({ address, restaurant, setMyRestaurantList, className }: Props) => {
      ...
      function displayMyRestaurant(restaurant) {
         ...
         var markers = restaurant.map(item => {
            ...
            return new window.kakao.maps.CustomOverlay({
               map: map,
               position: restaurant && new window.kakao.maps.LatLng(item.mapInfo.y, item.mapInfo.x),
               content: `
            <div class="customoverlay flex items-center gap-[6px] border border-1.5 border-primary bg-[#fff] p-[4px] pr-[8px]" style="min-width: 100px;  min-height: 11px; border-radius: 20px; position: relative; top: -9px; left: -0px; right:0;">
               ...
               <div class="text-[#333] text-[12px] ">
                  <p class=" leading-none" style="">${restaurant && item.mapInfo.name}</p>
                  <div class="flex items-center gap-[3px]" >
                     <div class="size-[8px] ">
                     ...
                     <p class="text-[10px] mt-[2px] leading-none">${restaurant && item.rating}</p>
                  </div>
               </div>
         ...
         // const customOverlay = new window.kakao.maps.CustomOverlay({
         //    map: map,
         //    position: restaurant && new window.kakao.maps.LatLng(restaurant[0].mapInfo.y, restaurant[0].mapInfo.x),
         //    content: `
         //    <div class="customoverlay flex items-center gap-[6px] border border-1.5 border-primary bg-[#fff] p-[4px] pr-[8px]" style="min-width: 100px;  min-height: 11px; border-radius: 20px; position: relative; top: -9px; left: -0px; right:0;">
         ...
         //       <div class="text-[#333] text-[12px] ">
         //          <p class=" leading-none" style="">${restaurant && restaurant[0].mapInfo.name}</p>
         //          <div class="flex items-center gap-[3px]" >
         //             <div class="size-[8px] ">
         //                <svg xmlns="http://www.w3.org/2000/svg" fill="#999" viewBox="0 0 13 13"><path d="M8.263 4.682h4.258a.479.479 0 0 1 .28.866L9.353 8.022l1.324 4.01a.477.477 0 0 1-.175.537.482.482 0 0 1-.563 0l-3.438-2.49-3.44 2.49a.48.48 0 0 1-.737-.538l1.324-4.01L.2 5.549a.477.477 0 0 1 .28-.867l4.258.001L6.044.675a.48.48 0 0 1 .912 0l1.307 4.007z"></path></svg>
         //             </div>
         //             <p class="text-[10px] mt-[2px] leading-none">${restaurant && restaurant[0].rating}</p>
         //          </div>
         //       </div>
   ...
   useEffect(() => {
      ...
      console.log('restaurant?', restaurant && restaurant[0])
   }, [restaurant])
